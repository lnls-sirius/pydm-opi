...
def proc_alarm(mod=True, std=False, timeout=1., error=True, prefix=None):
    if not prefix:
    ...
    if not _lock['working']:
        with _lock['lock']:
            ...
            for r in readings:
                ...
                pvs.append(pv)
            worker = threading.Thread(target=do_work, daemon=True, kwargs={'pvs':pvs, 'timeout':timeout,'_lock':_lock})
            worker.start()
